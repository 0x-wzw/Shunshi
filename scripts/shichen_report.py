"""Shichen Report Engine — Generic Temporal Analysis
=====================================================
Pure computation library. Takes birth date + gender as parameters.
Contains NO personal data — safe to commit to GitHub.

Produces a three-section daily temporal report:
  1. Shunshi four-stage decision flow (calibrate → simulate → tune → verify)
  2. 12-shichen hexagram grid with DM-element interaction + meridian guidance
  3. Daily synthesis with peak/warning windows

Usage:
    from shichen_report import ShichenReportEngine
    engine = ShichenReportEngine(datetime(1990, 5, 20, 10), gender='male')
    data = engine.compute_all()
    markdown = engine.format_markdown(data)
"""
...
ENGINE_PATH = os.path.expanduser("~/Shunshi/engine")
...
class ShichenReportEngine:
    """Generic temporal analysis engine. No personal data — birth_dt + gender passed as
    ...
    def __init__(self, birth_dt: datetime, gender: str = 'male'):
        ...
        self._shunshi = ShunShiEngine(birth_dt, gender=gender)
        self._hcalc = HexagramCalculator()
        self._bazi_calc = BaziCalculator()

        # Pre-compute natal reference data (constant per person)
        self._natal = self._bazi_calc.calculate(
            birth_dt.year, birth_dt.month, birth_dt.day, birth_dt.hour
        )
        self._dm = self._natal.day_master
        self._dm_elem = self._dm.element.name

    @property
    def natal_bazi_str(self) -> str:
        """八字 string: 己未 丙子 丁丑 乙巳"""
        return self._shunshi.bazi_summary()

    @property
    def day_master_str(self) -> str:
        """日主 description: 丁（FIRE阴）"""
        return f"{self._dm.character}（{self._dm_elem}{'阳' if self._dm.polarity.value == 1 else '阴'}）"

    # ── Computation ────────────────────────────────────────

    def compute_all(self, label: str = "") -> dict:
        """Run all computations once. `label` appears in report header (e.g. a name)."""
        ...
        cali     = self._shunshi.personal_calibration()
        sim      = self._shunshi.situational_simulation(question)
        env      = self._shunshi.environment_tuning()
        cups     = self._shunshi.intuition_verification()
        decision = self._shunshi.full_decision(question)

        # ── B. 12-Shichen Hexagram Grid ──
        # Tomorrow's day stem for 五鼠遁
        tomorrow = now + timedelta(days=1)
        tom_pillars = self._bazi_calc.calculate(
        ...
        for i, (shi_name, time_range, poetic, body_hint) in enumerate(SHICHEN_TABLE):
            ...
            hb = EARTHLY[i]
            h  = self._hcalc.day_pillar_to_hexagram(hs, hb, method="composite")
            rel = self._hcalc.trigram_element_relation(h.upper_trigram, h.lower_trigram)

            stem_tri = self._hcalc.STEM_TO_TRIGRAM.get(hs)
            energy_vs_dm = self._element_vs_dm(stem_tri.element) if stem_tri else "?"

            insight_text, xiangyi, tags = HEXAGRAM_INSIGHTS.get(h.number, ("", "", ""))
            advice = ELEMENT_ADVICE.get(rel, "保持平常心，随遇而安。")
            natal_interaction = self._natal_hexagram_interaction(h, rel)
        ...
        return {
            "meta": {
                "label": label, "date": today_str, "date_cn": today_cn,
                "bazi": self.natal_bazi_str, "gender": self.gender,
            },
            "shunshi": {
                "cali": cali, "sim": sim, "env": env, "cups": cups,
                "decision": decision,
            },
            "shichen": shichen_rows,
            "summary": {
                "peaks": peak_s, "warnings": warn_s,
                "dm_elem": self._dm_elem, "dm_char": self._dm.character,
            },
        }

    def _element_vs_dm(self, hour_elem: str) -> str:
        ...
        if prod.get(he) == self._dm_elem:    return "生我"
        if prod.get(self._dm_elem) == he:     return "我生"
        if ctrl.get(he) == self._dm_elem:     return "克我"
        if ctrl.get(self._dm_elem) == he:     return "我克"
        return "?"

    def _natal_hexagram_interaction(self, h, rel: str) -> str:
        ...
        if self._dm_elem in h_elems:
            return "卦象五行与日主同气，得心应手的时刻"
        if any(prod.get(e) == self._dm_elem for e in h_elems):
    ...
    def format_markdown(self, data: dict) -> str:
        ...
        # Header
        header_label = f" · {m['label']}" if m['label'] else ""
        lines.append("## 🌀 每日全息决断")
        lines.append(
            f"**{m['date_cn']}{header_label} · {m['bazi']} · {m['gender']}命**"
        )
        lines.append("")

        # ── Section A: Shunshi Four-Stage ──
        ...
        lines.append("### 🧘 个人校准")
        p = cal["四柱"]
        lines.append(f"**四柱**: 年{p['年']} 月{p['月']} 日{p['日']} 时{p['时']}")
        lines.append(f"**日主**: {cal['日主']}（{cal['日主五行']}{cal['日主阴阳']}）")
        lines.append(f"**简评**: {cal['日主简评']}")
        lines.append("**五行分布**: " + " | ".join(
            f"{e} {c}" for e, c in cal["五行分布"].items()
        ))
        lines.append("")
        ...
        # ── Section C: Daily Synthesis ──
        lines.append("---")
        lines.append("")
        lines.append("## 三、全日总评")
        lines.append("")
        lines.append(f"**日主**: {su['dm_char']}（{ELEM_CN.get(su['dm_elem'], su['dm_elem'])}）")
        ...
        lines.append("---")
        lines.append("*每日 23:00 UTC 自动生成 · Shunshi Engine 顺势 × 时辰 × 养生*")
